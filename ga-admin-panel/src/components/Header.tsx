...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white border-b border-slate-100/80 sticky top-0 z-30 px-4 sm:px-6 h-16 flex items-center justify-between shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
      {/* Left side: Hamburger (3 lines) + Title */}
      <div className="flex items-center gap-2.5 sm:gap-3">
        <button
          id="btn-sidebar-toggle"
          onClick={onToggleSidebar}
          aria-label={sidebarOpen ? 'Close sidebar and make page full width' : 'Open sidebar'}
          title={sidebarOpen ? 'Close sidebar (Full screen)' : 'Open sidebar'}
          className={`p-2 -ml-1 rounded-lg transition-all duration-200 focus:outline-none cursor-pointer group ${
            !sidebarOpen
              ? 'bg-blue-50 text-blue-600 ring-1 ring-blue-200/80 shadow-xs'
              : 'text-slate-700 hover:text-blue-600 hover:bg-slate-100'
          }`}
        >
          <Menu className="w-5 h-5 sm:w-6 sm:h-6 stroke-[2.3] transition-transform duration-200 group-hover:scale-105" />
        </button>
